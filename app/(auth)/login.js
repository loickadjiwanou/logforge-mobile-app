...
import { Rocket, Mail, Lock, Languages, Sun, Moon } from 'lucide-react-native';
...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    if (!email || !password) {
      Alert.alert(t('errorLoginFailed'), t('errorFillFields'));
      return;
    }
    ...
    try {
      ...
      if (res.data.access_token) {
        ...
        await login(res.data.access_token, userRes.data);
        router.replace('/(main)/(tabs)/dashboard');
      }
    } catch (error) {
      console.log('Login error:', error.response?.data || error.message);
      Alert.alert(t('errorLoginFailed'), error.response?.data?.detail || 'Invalid credentials');
    } finally {
      setLoading(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.actions}>
          <TouchableOpacity onPress={() => router.push('/(auth)/signup')}>
            <Text style={[styles.actionText, { color: primaryColor }]}>{t('dontHaveAccount')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
